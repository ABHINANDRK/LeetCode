...
class BrowserHistory(object):
    ...
    def __init__(self, homepage):
        ...
        self.currentPosition = Node(homepage)

    def visit(self, url):
        node = Node(url)

        if self.currentPosition.next == None:
            self.currentPosition.next = node
            node.prev = self.currentPosition
            self.currentPosition = node
        else:
            temp = self.currentPosition.next
    ...
    def back(self, steps):
        temp = self.currentPosition
        head = None

        while(steps > 0 and temp != None):
            head = temp
            temp = temp.prev
            steps = steps - 1
        ...
        if temp != None:
            ...
            self.currentPosition = head
            return self.currentPosition.url
